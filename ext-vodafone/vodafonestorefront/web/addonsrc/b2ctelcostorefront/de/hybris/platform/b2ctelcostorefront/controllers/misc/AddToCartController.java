...
public class AddToCartController extends AbstractController
{
			...
			final String bundleTemplateId)
	{
		try
		{
			final List<CartModificationData> cartModifications = getBundleCartFacade().addToCart(code, qty, bundleNo.intValue(),
			...
			@RequestParam(value = "bundleTemplateId2", required = false) final String bundleTemplateId2)
	{
		...
		try
		{
			final List<CartModificationData> cartModifications = getBundleCartFacade().addToCart(productCode1, -1, bundleTemplateId1,
	...
	protected void setBundleCartFacade(final BundleCartFacade bundleCartFacade)
	{
