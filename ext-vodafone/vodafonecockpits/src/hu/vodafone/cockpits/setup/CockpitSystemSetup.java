 ...
 */
package hu.vodafone.cockpits.setup;
...
import hu.vodafone.cockpits.constants.VodafoneCockpitsConstants;
 ...
 */
@SystemSetup(extension = VodafoneCockpitsConstants.EXTENSIONNAME)
public class CockpitSystemSetup extends AbstractSystemSetup
{
	...
	public void createEssentialData(final SystemSetupContext context)
	{
		...
		{
			if (MediaManager.getInstance().getMediaFolderByQualifier(VodafoneCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER).size() < 1)
			{
				MediaManager.getInstance().createMediaFolder(VodafoneCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER,
						VodafoneCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER);
			}
			try
			{
				String prefix = null;
				...
				if (prefix != null){
					importImpexFile(context, "/vodafonecockpits/reportcockpit/import/" + prefix + "_jasperreports.impex");
				}
			}
